...
class Checkpoint_Selector:
    ...
    def select_best_checkpoint(self, project_dir: str, metric: str = "mAP50") -> str | None:
        """
        ...
        if last_checkpoint.exists():
            self.logger.warning(
                f"Best checkpoint not found, using last checkpoint: {last_checkpoint}"
            )
            return str(last_checkpoint)

        # If no checkpoints found, raise error
        raise FileNotFoundError(
            f"No checkpoints found in {project_path / 'weights'}. Expected 'best.pt' or 'last.pt'"
        )
