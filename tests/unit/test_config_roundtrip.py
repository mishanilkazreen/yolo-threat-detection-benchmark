...
def training_config_strategy(draw):
    ...
    seeds = None
    if draw(st.booleans()):
        seeds = draw(st.lists(st.integers(min_value=0, max_value=10000), 
...
def model_config_strategy(draw):
    ...
    return ModelConfig(
        ...
        weights=draw(st.text(min_size=1, max_size=50, alphabet=st.characters(
...
def data_config_strategy(draw):
    ...
    return DataConfig(
        yaml_path=draw(st.text(min_size=1, max_size=100, alphabet=st.characters(
            whitelist_categories=('Lu', 'Ll', 'Nd'), whitelist_characters='.-_/')))
    )
